...
io.on("connection", (socket) => {
    ...
    socket.on("join_channel", (data) => {
        ...
        users[socket.id] = { name: userName || "Unknown", channel: channelId };
        console.log(`User ${users[socket.id].name} joined channel: ${channelId}`);
    ...
    socket.on("send_audio", (data) => {
        console.log("Data received from user:", socket.id);
        ...
        if (data && data.channelId && data.channelId !== "undefined" && data.audio) {
            ...
            console.log(`Audio broadcasted to channel: ${data.channelId}`);
        } else {
